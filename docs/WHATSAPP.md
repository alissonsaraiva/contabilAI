...
| Delete usava remoteJid da conversa ao invés da key | ✅ Resolvido v3.10.63 | Números brasileiros têm formato diferente entre conversa (`5585981186338`) e key da Evolution (`558581186338` — sem o 9). REVOKE com JID errado é ignorado silenciosamente pelo WhatsApp (API retorna 201 sucesso). Fix: usar `waKey.remoteJid` com fallback para `conversa.remoteJid`. **Padrão:** nunca assumir que conversa.remoteJid === waKey.remoteJid. |
