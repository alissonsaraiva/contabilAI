...
const buscarChamadoTool: Tool = {
  definition: {
    name: 'buscarChamado',
    description:
      'Busca um chamado específico pelo número (#42), pelo UUID ou por trecho do título. Use quando o cliente ou operador mencionar um número de chamado (ex: "meu chamado #12", "chamado número 5") ou quando precisar dos detalhes de um chamado específico antes de responder ou atualizar.',
    inputSchema: {
      type: 'object',
      properties: {
        numero: {
  ...
  async execute(input: Record<string, unknown>, ctx: ToolContext): Promise<ToolExecuteResult> {
    ...
    if (numero !== undefined || id) {
      ...
      const os = await prisma.chamado.findFirst({
        where,
        include: {
...
registrarTool(buscarChamadoTool)
