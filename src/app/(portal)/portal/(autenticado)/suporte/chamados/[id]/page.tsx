...
import { ChamadoAvaliacaoForm } from '@/components/portal/chamado-avaliacao-form'
import { ChamadoCancelarBtn } from '@/components/portal/chamado-cancelar-btn'
...
export default async function ChamadoDetailPage({ params }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[12px] text-on-surface-variant/60">
            Chamado #{id.slice(-8)} · {TIPO_CHAMADO[ordem.tipo] ?? ordem.tipo}
          </p>
        </div>
      </div>
      ...
      {/* Avaliação */}
      {podeAvaliar && <ChamadoAvaliacaoForm ordemId={id} />}
      {ordem.avaliacaoNota && (
        <Card className="border-outline-variant/15 bg-card/60 p-4 rounded-[16px] shadow-sm">
          <div className="flex items-center gap-3">
