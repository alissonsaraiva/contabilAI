  ...
  try {
    ...
    if (!email || !EMAIL_REGEX.test(email)) {
      return NextResponse.json(
        { error: 'E-mail do lead inválido ou não encontrado. Preencha o campo E-mail no onboarding.' },
        { status: 400 },
      )
    }
    ...
    console.error('[contrato-enviar] Erro ao enviar contrato:', err)
    Sentry.captureException(err, {
      tags: { module: 'contrato-enviar', operation: 'enviar-para-assinatura' },
      extra: { leadId: id },
    })
    return NextResponse.json({ error: `Erro ao enviar contrato: ${msg}` }, { status: 500 })
  }
}
