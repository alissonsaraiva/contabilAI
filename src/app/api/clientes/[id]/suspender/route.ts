...
export async function POST(req: Request, { params }: Params) {
  ...
  import('@/lib/services/asaas-sync')
    ...
    .catch((err: unknown) => {
      console.error('[suspender] Erro ao cancelar subscription no Asaas — cliente pode continuar sendo cobrado:', err)
      Sentry.captureException(err, {
        tags:  { module: 'clientes-api', operation: 'suspender-asaas' },
        extra: { clienteId: id },
      })
    })
