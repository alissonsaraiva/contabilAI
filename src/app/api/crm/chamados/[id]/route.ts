...
import { resolverOS } from '@/lib/services/chamados'
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  // Nota interna: salva separado, não vai para o cliente
  if (body.nota_interna?.trim()) {
    await prisma.chamadoNota.create({
      data: {
        chamadoId: id,
        conteudo:  body.nota_interna.trim(),
        autorId:   usuarioId ?? null,
      },
    })
  }
  ...
  if (body.status === 'resolvida' && existing.status !== 'resolvida') {
    if (existing.clienteId) {
      sendPushToCliente(existing.clienteId, {
        ...
        console.error('[crm/chamados] erro ao enviar push chamado resolvido:', { clienteId: existing.clienteId, err }),
      )
    }
  }
