...
type Props = { tarefa: Tarefa; clientes: Cliente[]; children: React.ReactNode }

export function EditarTarefaDrawer({ tarefa, clientes, children }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.titulo.trim() || form.titulo.length < 3) return
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-full sm:max-w-md flex flex-col gap-0 p-0 bg-card" showCloseButton={false}>
        ...
        <div className="flex items-center gap-3 border-b border-outline-variant/15 px-6 py-5">
          ...
          <div>
            <h2 className="text-base font-semibold text-on-surface">Editar Tarefa</h2>
            <p className="text-[12px] text-on-surface-variant">Atualize os detalhes da tarefa</p>
          </div>
        </div>
