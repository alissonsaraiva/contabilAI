...
import { ListasTab, ListaDetalhe } from './listas'
import type { ListaResumo } from './listas'

type TopTab = 'conversas' | 'listas'
...
export function AtendimentosWeb({
  ...
  // Determinar se o painel direito tem algo selecionado
  const hasRightPanel = topTab === 'conversas' ? !!selected : !!selectedLista
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      {/* ─── Painel esquerdo ─────────────────────────────────────────── */}
      <div className={`flex shrink-0 flex-col overflow-hidden border-r border-outline-variant/15 bg-card w-full lg:w-80 ${hasRightPanel ? 'hidden lg:flex' : 'flex'}`}>

        {/* Top tabs: Conversas / Listas */}
        <div className="flex border-b border-outline-variant/15">
          <button
            onClick={() => handleTopTabChange('conversas')}
            className={`flex flex-1 items-center justify-center gap-1.5 py-3 text-[12px] font-semibold transition-colors ${
              topTab === 'conversas'
                ? 'border-b-2 border-primary text-primary'
                : 'text-on-surface-variant/50 hover:text-on-surface'
            }`}
          >
            <span className="material-symbols-outlined text-[16px]" style={{ fontVariationSettings: "'FILL' 1" }}>forum</span>
            Conversas
            {totalConversas > 0 && (
              <span className="rounded-full bg-surface-container px-1.5 text-[10px] font-bold text-on-surface-variant">{totalConversas}</span>
            )}
          </button>
          <button
            onClick={() => handleTopTabChange('listas')}
            className={`flex flex-1 items-center justify-center gap-1.5 py-3 text-[12px] font-semibold transition-colors ${
              topTab === 'listas'
                ? 'border-b-2 border-primary text-primary'
                : 'text-on-surface-variant/50 hover:text-on-surface'
        ...
        {topTab === 'conversas' ? (
          ...
          /* ─── Aba Listas ─────────────────────────────────────────── */
          <ListasTab
            onSelectLista={lista => setSelectedLista(lista)}
            selectedListaId={selectedLista?.id ?? null}
          />
        )}
      </div>

      {/* ─── Painel direito ──────────────────────────────────────────── */}
      <div className={`flex-col overflow-hidden bg-surface-container-low ${hasRightPanel ? 'flex flex-1' : 'hidden lg:flex lg:flex-1'}`}>
        {topTab === 'conversas' ? (
          selected ? (
            ...
            <PlaceholderPainel onNova={() => setNovaConversa(true)} />
          )
        ) : (
          selectedLista ? (
            <ListaDetalhe
              ...
              onDeleted={() => setSelectedLista(null)}
            />
          ) : (
            <PlaceholderListas />
          )
...
function PlaceholderPainel({ onNova }: { onNova: () => void }) {
  return (
...
function PlaceholderListas() {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-4 text-center">
      <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-card shadow-sm">
        <span className="material-symbols-outlined text-[32px] text-on-surface-variant/25" style={{ fontVariationSettings: "'FILL' 1" }}>campaign</span>
      </div>
      <div>
        <p className="text-[14px] font-medium text-on-surface-variant/60">Selecione uma lista</p>
        <p className="mt-1 text-[12px] text-on-surface-variant/30">Crie ou selecione uma lista para gerenciar membros e enviar mensagens</p>
      </div>
    </div>
  )
}
