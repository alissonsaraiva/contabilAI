...
async function main() {
  ...
  const semana = new Date(agora); semana.setDate(agora.getDate() + 5)
  const mesQueVem = new Date(agora); mesQueVem.setDate(agora.getDate() + 20)
  ...
  await prisma.tarefa.createMany({
    ...
    data: [
      {
        id: 'tarefa-001',
        titulo: 'Enviar DCTF de setembro — JP Almeida',
        descricao: 'Declaração de Débitos e Créditos Tributários Federais, prazo improrrogável.',
        prioridade: 'urgente',
        status: 'pendente',
        prazo: ontem,
        clienteId: c1.id,
      ...
      {
        id: 'tarefa-002',
        titulo: 'Revisar folha de pagamento — Construtora Arco',
        descricao: 'Verificar horas extras e descontos do período antes de processar.',
        prioridade: 'alta',
        status: 'em_andamento',
        prazo: amanha,
        clienteId: c3.id,
      ...
      {
        id: 'tarefa-003',
        titulo: 'Contato com inadimplente — Menu Taste',
        descricao: 'Cliente com 2 mensalidades em aberto. Enviar proposta de regularização.',
        prioridade: 'alta',
        status: 'pendente',
        prazo: amanha,
        clienteId: c2.id,
      ...
      {
        id: 'tarefa-004',
        titulo: 'Gerar DAS de outubro — Sandra Oliveira',
        descricao: 'Geração do DAS MEI referente ao mês de outubro.',
        prioridade: 'media',
        status: 'pendente',
        prazo: semana,
        responsavelId: contador.id,
      ...
      {
        id: 'tarefa-005',
        titulo: 'Relatório DRE mensal — NextTech',
        descricao: 'Preparar demonstração de resultado do exercício para reunião com investidores.',
        prioridade: 'alta',
        status: 'em_andamento',
        prazo: semana,
        clienteId: c3.id,
      ...
      {
        id: 'tarefa-006',
        titulo: 'Onboarding novo cliente — JP Almeida',
        descricao: 'Coletar documentos iniciais e configurar acesso ao portal do cliente.',
        prioridade: 'media',
      ...
      {
        id: 'tarefa-007',
        titulo: 'Renovação de certificado digital — Construtora Arco',
        descricao: 'Certificado e-CNPJ vence em 30 dias. Agendar renovação com o cliente.',
        prioridade: 'media',
        status: 'pendente',
        prazo: mesQueVem,
        clienteId: c3.id,
      ...
      {
        id: 'tarefa-008',
        titulo: 'Revisão contratual anual — Fernanda Lima',
        descricao: 'Verificar reajuste de acordo com INPC e enviar proposta de renovação.',
        prioridade: 'baixa',
        status: 'pendente',
        prazo: mesQueVem,
        clienteId: c2.id,
  ...
  // Tarefas concluídas este mês para mostrar no contador
  await prisma.tarefa.createMany({
    skipDuplicates: true,
    data: [
      { id: 'tarefa-009', titulo: 'SPED Fiscal enviado — JP Almeida', prioridade: 'alta', status: 'concluida', clienteId: c1.id, responsavelId: contador.id },
      { id: 'tarefa-010', titulo: 'Simei anual gerado — Sandra Oliveira', prioridade: 'media', status: 'concluida', responsavelId: contador.id },
      { id: 'tarefa-011', titulo: 'Balancete fechado — Construtora Arco', prioridade: 'alta', status: 'concluida', clienteId: c3.id, responsavelId: admin.id },
      { id: 'tarefa-012', titulo: 'Relatório de sócios — NextTech', prioridade: 'media', status: 'concluida', clienteId: c3.id, responsavelId: admin.id },
    ],
  })
